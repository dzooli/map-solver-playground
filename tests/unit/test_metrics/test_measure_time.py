...
class TestMeasureTime:
    def test_measure_time_decorator_returns_function_result(self):
        ...
        with patch("map_solver_playground.metrics.timing.time.time", side_effect=[0, 1]) as mock_time:
            @measure_time
            def sample_func():
                return 42
    ...
    def test_measure_time_decorator_records_execution_time(self):
        # Mock time.time to return specific values
        with patch("map_solver_playground.metrics.timing.time.time", side_effect=[0, 2]) as mock_time:
            @measure_time
            def slow_func():
                return "done"
    ...
    def test_measure_time_decorator_with_kwargs(self):
        # Mock time.time to return specific values
        with patch("map_solver_playground.metrics.timing.time.time", side_effect=[0, 0.5]) as mock_time:
            @measure_time
